...
### GraphQL relation eager-loading

`src/utils/graphql-selection.util.ts` exports `getRequestedRelations(info: GraphQLResolveInfo, metadata: EntityMetadata): string[]`, which walks the current query's selection set against an entity's TypeORM relation metadata and returns exactly the relation paths the query selected — including relations-of-relations as dotted paths (e.g. `comments.author.posts`), capped at `MAX_RELATION_DEPTH` (`src/constants/common.ts`, shared by every domain so the cap behaves consistently everywhere) to bound how deep — and how much row fan-out — a single query can trigger. Every domain's service that backs a root query exposes an `entityMetadata` getter (`get entityMetadata() { return this.xRepository.metadata; }` — see `PostsService`, `CommentsService`, `UsersService`, `CategoriesService`, `PaymentsService`) and every root query accepts `@Info() info: GraphQLResolveInfo` and forwards `getRequestedRelations(info, this.xService.entityMetadata)` into the service call's `relations` option: `PostsResolver.findAll/findOne`, `CommentsResolver.commentsByPost`, `UsersResolver.findAll/findOne/me`, `CategoriesResolver.findAll`, `PaymentsResolver.myTransactions/transactionsForPost`. This lets one query eager-load whatever was actually requested instead of firing one query per `@ResolveField`. Correspondingly, every `@ResolveField` a relation could satisfy (`PostsResolver.getAuthor/getCategories/getOpenGraphMetadata/getPostImage`, `CommentsResolver.getPost/getAuthor`, `UsersResolver.getPosts/getAvatar`, `CategoriesResolver.getPosts`, `PaymentsResolver.getPost/getUser`) checks `parent.<relation> !== undefined` first and only falls back to a DB call when the relation wasn't eager-loaded — the check is `!== undefined`, not `??`, because a loaded-but-empty relation is legitimately `null` and re-querying it would defeat the point. `ChatResolver` and `OpenGraphResolver` are exempt: their entities either have no `@ResolveField`-backed relations or use TypeORM's `{ eager: true }` on the relation itself (`ChatMessageEntity.sender`/`attachments`), so there's no N+1 to eliminate there.
